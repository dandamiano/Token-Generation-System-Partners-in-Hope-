...
<div class="container-fluid">
    ...
    <thead>
        <tr>
            ...
            <th>Status</th>
            <th>Token Status</th> <!-- New column for token status -->
        </tr>
    </thead>
...
function updateTokenStatus(appointmentId, tokenStatus) {
    $.ajax({
        url: 'update-token-status.php',  // PHP file to update token status
        method: 'POST',
        data: { appointment_id: appointmentId, token_status: tokenStatus },
        success: function(response) {
            alert(response); // Show success message from PHP (if any)
            
            if (tokenStatus === 'called') {
                // If the status is 'called', display an alert to the patient
                alert("Your appointment has been called. Please proceed to the doctor.");
            }

            // Reload appointments dynamically (if applicable)
            loadAppointments();

            // Reload status counts dynamically (if applicable)
